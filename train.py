...
def train(train_data_path, validation_data_path, num_epochs, out_path, loss_fn, model: AbstractNetwork,
          optimizer: torch.optim.Optimizer, batch_size=1, verbose=True, initial_lr=1e-3,
          transformations={"train": Compose([ToTensor()]), "validate": Compose([ToTensor()])}, **kwargs):
    ...
    img_size = (224, 224)
    trainset = ShapeDataset(train_data_path, transformations["train"], img_size)
    validation_set = ShapeDataset(validation_data_path, transformations["validate"], img_size)
    ...
    translation_params = torch.from_numpy(np.asarray(img_size)/2).float().to(device)
    ...
    for epoch in range(num_epochs):
        ...
        optimizer, model = model.single_epoch(train_loader, optimizer, model, loss_fn,
                                              translation_params=translation_params, device=device, verbose=verbose,
                                              n_recurrent_iters=2, img_size=min(img_size), curr_epoch=epoch,
                                              writer=writer, **kwargs)

        with torch.no_grad():
            print("\tValidating")
            model.validate(validation_loader, model, loss_fn, device=device, verbose=verbose,
                           translation_params=translation_params, n_recurrent_iters=2, img_size=min(img_size),
                           curr_epoch=epoch, out_dir=curr_out_dir, writer=writer, **kwargs)

        save_network(model, optimizer, epoch, os.path.join(out_path, "Checkpoints", "model_epoch_%d.pth" % (epoch+1)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-u", "--user", help="Select User", dest="user", type=str, default="schock")
    ...
    parse_args = sys.argv[1:] if len(sys.argv) > 1 else ["-u", "schock", "-f", yaml_file_path]
    parsed_args = parser.parse_args(parse_args)

    config = NetConfig(parsed_args.config_file, parsed_args.user)

   #  eigen_shapes = DataProcessing.from_dir(config.pca_data_path).lmk_pca(config.scale, config.center,
